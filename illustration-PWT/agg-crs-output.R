...
for (i in 1:length(year)) {
  ...
  tt=coverage.agg.crs.illu(x=xi,y=yi,py=pyi,L=20)
...
outfile=paste("./Data/result",np,"q",nq,".csv",sep="")
write.csv(res0,row.names=FALSE,file=outfile)

outfile=paste("./Data/result-sharpening-p",np,"q",nq,".csv",sep="")
...
### construct the Table With Data Sharpeing ###
tex=formatC(year,width=6,digits=0,format="f")
for (k in 1:ncol(res)) {
...
setEPS()
postscript(file = "./Figures/pwt-sigma-agg-100kappa.eps",width=10, height=8)
...
postscript(file = "./Figures/pwt-CI-agg-100kappa.eps",width=10, height=8)
...
setEPS()
postscript(file = "./Figures/pwt-meanbias-agg-100kappa.eps",width=10, height=8)
